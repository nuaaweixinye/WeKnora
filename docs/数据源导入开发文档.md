...
##### docx 块型支持清单（blocks 模式）

| 类别 | 块型（飞书 block_type） | 渲染结果 |
|------|------------------------|---------|
| 文本结构 | 页面(1)、文本(2)、标题 H1-H6(3-8)、无序/有序列表(12/13)、任务列表(17)、引用(15)、分隔线(22) | 标准 Markdown（任务列表为 GFM `- [ ]` 语法） |
| 深层标题 | H7-H9(9-11) | 粗体文本（GFM 标题上限 H6） |
| 代码块(14) | 保留语言别名（约 75 种语言映射），加长围栏防代码内 ``` 提前闭合 | ``` 围栏代码块 |
| 容器 | 高亮块(19)、引用容器(34)：整块内容按 `>` 引用渲染；分栏(24/25)：按列顺序输出、列间 `---`；视图(33)：静默渲染子块 | |
| 表格(31) | GFM 表格；**合并单元格拆分为独立单元格并填充左上值**；列对齐随源文档（style.align → `:---:` / `---:`） | |
| 内嵌电子表格(30) / 多维表格(18) | **转为文档内 Markdown 表格**（值接口读取，合并单元格以左上值填充），检索时保持文档上下文 | |
| 图片(27) / 画板(43，含思维导图) | 编号占位标记 → 同步时下载字节以 base64 内联 → 处理管线换 WeKnora 持久图片地址；画板经 `download_as_image` 导出（自动裁白边） | |
| 文件附件(23) | 白名单类型（pdf/md/xlsx/xls/csv/doc/docx/ppt/pptx）下载后作为独立知识入库（与父文档同目录、元数据互记），正文以附件名列表展示；其余类型留带源链接的引用 | |
| 内嵌网页(26) | `[内嵌网页](URL)` | |
| 文档小组件(40) | 时间轴组件转 Markdown 列表（遵循 contentShow 显示设置）；其余组件类型占位 | |
| 降级占位 | 会话卡片(20)、流程图&UML(21)、开放平台小组件(28)、思维笔记(29)、任务(35)、OKR(36-39)、Jira(41)、Wiki 子页面列表(42/51)、议程(44-47)、链接预览(48)、同步块(49/50)、AI 模板(52) | `> [飞书块: 名称]` 一行注记，不影响文档其余内容 |
